...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.trigger.after [:up, :reload], :stdout => true, :force => true do
    run 'ansible-playbook -i ' + boxipaddress + ', -K --user=vagrant --private-key=' + vagrant_dir + '/.vagrant/machines/vagrant/' + boxname + '/private_key ' + vagrant_dir + '/provision/playbooks/virtual_hosts.yml --extra-vars "local_ip_address=' + boxipaddress + '"'
  end
